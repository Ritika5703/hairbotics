const NavbarStyles = {
  navbar: "bg-white text-black py-4 shadow-md",
  container:
    "container mx-auto flex flex-col md:flex-row justify-between items-center px-4 md:px-6",
  logo: "text-2xl font-bold text-green-500 mb-4 md:mb-0",
  toggleButton: "text-2xl text-green-500 focus:outline-none",
  navLinks:
    "flex flex-col md:flex-row md:space-x-6 space-y-4 md:space-y-0 absolute md:relative bg-white md:bg-transparent w-full md:w-auto transition-all duration-300",
  openMenu: "top-16 p-4",
  closedMenu: "top-[-200px] md:top-auto hidden md:flex",
  link: "hover:text-green-500",
  actionButtons: "mt-4 md:mt-0 flex space-x-4",
  getStartedButton:
    "bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 mx-2",
  loginButton:
    "bg-transparent text-green-500 border border-green-500 px-4 py-2 rounded-lg hover:bg-green-100",
};

export default NavbarStyles;
